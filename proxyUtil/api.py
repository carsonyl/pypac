...
from proxyUtil.parser import PACFile
from proxyUtil.resolver import ProxyResolver, ProxyConfigExhaustedError
from proxyUtil.os_settings import autoconfig_url_from_registry, autoconfig_url_from_preferences, \
    ON_WINDOWS, ON_DARWIN, file_url_to_local_path
from proxyUtil.wpad import proxy_urls_from_dns
...
class virtualProxyEnviron():
    ...
    def unset_environ(self):
        if self.prev_http_proxy:
            ...
            del os.environ['HTTP_PROXY']
        if self.prev_https_proxy:
            os.environ['HTTPS_PROXY'] = self.prev_https_proxy
        elif 'HTTPS_PROXY' in os.environ:
            del os.environ['HTTPS_PROXY']
